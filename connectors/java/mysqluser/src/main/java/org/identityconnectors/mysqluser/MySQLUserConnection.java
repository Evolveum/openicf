...
/**
 * The MySQLUserConnection extends the DatabaseConnection overriding the test
 * method.
 *
 * @since 1.0
 */
public final class MySQLUserConnection extends DatabaseConnection {
    ...
    private static final String VALIDATE_CONNECTION = "FLUSH STATUS";
    /**
     * The configuration.
     */
     ...
     * database.
     *
     * If the {@link Connection} fails a {@link RuntimeException} will
     * be thrown.
     *
     * @param conn
     *            Real connection.
     ...
     *
     * @param config
     *            a {@link MySQLUserConfiguration} configuration object
    ...
    private static java.sql.Connection getNativeConnection(MySQLUserConfiguration config) {
        ...
        if (StringUtil.isNotBlank(datasource)) {
            ...
            connection =
                    SQLUtil.getDriverMangerConnection(config.getDriver(), MySQLUserConfiguration
                            .getUrlString(config.getHost(), config.getPort()), config.getUser(),
                            config.getPassword());
        }
