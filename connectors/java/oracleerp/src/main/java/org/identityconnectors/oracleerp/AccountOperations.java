...
import static org.identityconnectors.oracleerp.OracleERPUtil.PERSON_PARTY_ID;
...
import static org.identityconnectors.oracleerp.OracleERPUtil.SESS_NUM;
...
final class AccountOperations {
     ...
     * The map of column name parameters mapping.
     */
    public static final Map<String, CallParam> PARAM_MAP = CollectionUtil
            .<CallParam> newCaseInsensitiveMap();
    ...
    static {
        ...
        CALL_PARAMS.add(new CallParam(OWNER, "x_owner => upper({0})", Types.VARCHAR)); // 2
                                                                                       // write
                                                                                       // only
        CALL_PARAMS.add(new CallParam(UNENCRYPT_PWD, "x_unencrypted_password => {0}", Types.NULL));// 3
                                                                                                   // write
                                                                                                   // only
        ...
        CALL_PARAMS.add(new CallParam(PWD_ACCESSES_LEFT, "x_password_accesses_left => {0}",
                Types.INTEGER)); // 10
        CALL_PARAMS.add(new CallParam(PWD_LIFESPAN_ACCESSES, "x_password_lifespan_accesses => {0}",
                Types.INTEGER)); // 11
        CALL_PARAMS.add(new CallParam(PWD_LIFESPAN_DAYS, "x_password_lifespan_days => {0}",
                Types.INTEGER)); // 12
