...
public class SolarisAuthenticate extends AbstractOp {
    ...
    private static final Log logger = Log.getLog(SolarisAuthenticate.class);
    ...
    public Uid authenticate(ObjectClass objectClass, String username, GuardedString password,
        ...
        SolarisUtil.controlObjectClassValidity(objectClass, acceptOC, getClass(), connection
                .getConfiguration());
        logger.info("authenticate (user: '{0}')", username);
        ...
        // in case we logged in with different user that the root user, the
        // following applies:
        ...
        if (!configuration.isSudoAuthorization() && configuration.isSuAuthorization()) {
            connection.executeCommand("exit", Collections.<String> emptySet(), CollectionUtil
                    .newSet(loginShellPrompt));
        }
        ...
        logger.info("authenticate successful for user: '{0}'", username);
    ...
    private Map<String, SolarisConnection.ErrorHandler> initRejectsMap(final String username) {
        ...
        "incorrect", new SolarisConnection.ErrorHandler() {
            public void handle(String buffer) {
                throw new InvalidCredentialException("Incorrect authentication for user: "
                        + username + "");
            }
        },
        ...
        // this is a workaround for unit tests, as they rely on the right
        // exception message.
