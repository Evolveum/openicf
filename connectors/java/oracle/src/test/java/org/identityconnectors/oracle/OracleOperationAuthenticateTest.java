...
public class OracleOperationAuthenticateTest extends OracleConnectorAbstractTest {
	...
	private void testAuthenticate(ConnectorFacade facade) {
        ...
        AssertJUnit.assertEquals("Uid returned from authenticate must be same as returned from create ", uid, aUid);
        ...
        try{
        	...
        	Assert.fail("Authenticate with expired accoun must throw PasswordExpiredException only :" + e);
        }
    ...
    public void testReturnUidOnly(){
    	...
    	AssertJUnit.assertEquals("Uid returned from authenticate must be same as returned from create ", uid, aUid2);
