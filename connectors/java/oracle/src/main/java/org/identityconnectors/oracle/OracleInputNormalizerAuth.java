...
/**
 *
 * Input normalizer normalizes input attributes from create/update/remove and
 * authenticate. Does not normalizes output attributes
 *
 * @author kitko
 *
 */
final class OracleInputNormalizerAuth implements OracleAttributeNormalizer {
    ...
    OracleInputNormalizerAuth(OracleCaseSensitivitySetup cs) {
        this.cs = cs;
    }
    ...
    Attribute normalizeAttribute(ObjectClass oclass, Class<? extends SPIOperation> op,
            Attribute attribute) {
        if (attribute == null) {
            return null;
        }
